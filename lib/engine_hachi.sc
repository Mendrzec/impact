...
SynthDef("kick", {
      ...
      kill_gate = kill_gate + Impulse.kr(0);
		  kill_envelope = EnvGen.kr(envelope: Env.asr( 0, 1, 0.01), gate: kill_gate, doneAction: Done.freeSelf);
...
SynthDef.new("snare", {
	arg snare_trigger, snare_level, snare_tone, tone2=189, snare_snappy, snare_decay;
...
// This is how you add "commands", how the lua interpreter controls the engine.
		...
		this.addCommand("kick_trigger_2", "fff", {
		  arg msg;
		  var level = if (msg[1] != nil, {msg[1]},{kick_level});
		  var tone = if (msg[2] != nil, {msg[2]},{kick_tone});
		  var decay = if (msg[3] != nil, {msg[3]},{kick_decay});
		  if (kick_voice != nil, {
			  kick_voice.set(\kill_gate, 0);
		  });
		  kick_voice = Synth("kick", [\out, context.out_b, \kick_decay,decay,\kick_tone,tone,\kick_level,level], target:pg);
		});
    ...
    this.addCommand("oh_level", "f", {arg msg;
      oh_level = msg[1];
		...
		this.addCommand("snare_trigger", "f", {
		  arg msg;
		  var val = msg[1];
		  Synth("snare", [\out, context.out_b, \snare_trigger,val,\snare_tone,snare_tone,\snare_snappy,snare_snappy,\snare_level,snare_level,\snare_decay,snare_decay], target:pg);
		});
